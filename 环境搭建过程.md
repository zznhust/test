...
4. 安装 xformers 高效 attention

```bash
git clone -b v0.0.20 https://github.com/facebookresearch/xformers.git
cd xformers
...
```bash
https://pan.baidu.com/s/1HPbRRE5ZtPRpOSocm9qOmA?pwd=BA1c
```

6. 下载**ICM-57**数据集

```bash
https://pan.baidu.com/s/1ZJU_XHEVhIaVzGFPK_XCRg?pwd=BA1c
```

此处注意ICM-57格式必须满足如下样式，同时必须下载对应的trimap文件
...
7. 运行验证命令，得到预测结果和损失结果情况

```bash
python eval.py --checkpoint 12-0.00800-mat.pth --save_path results/ --config config/eval.yaml
```
